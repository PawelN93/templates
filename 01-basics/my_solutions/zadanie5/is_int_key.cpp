...
class VectorMap{
    ...
    public:
        ...
        T2& operator[](T1 key);
        bool isIntKey(){
            //return std::is_integral<T1>::value; mozliwa konwersja z char na int
            return std::is_same<T1, int>::value;    //tylko int 
        }

        static constexpr bool is_int_key = std::is_same<T1, int>::value;
};
...
int main(){
    ...
    std::cout<<std::boolalpha<<mapC.isIntKey()<<std::endl;
    std::cout<<std::boolalpha<<VectorMap<float, char>::is_int_key<<std::endl;
